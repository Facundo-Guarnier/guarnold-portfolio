...
const FALLBACK_IMAGE = 'https://images.unsplash.com/photo-1557683316-973673baf926?q=80&w=2029&auto=format&fit=crop';
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  ...
  const cardBaseClasses = "group relative flex flex-col h-full rounded-3xl bg-surface-variant transition-all duration-300 border border-outline/50 hover:border-primary no-underline overflow-hidden hover:shadow-xl hover:-translate-y-1";

  const statusBadgeClasses = "absolute top-3 right-3 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/40 backdrop-blur-md border border-white/20 text-white text-[10px] uppercase font-bold tracking-wider shadow-lg z-20";
  ...
  return (
    <a 
      ...
      className={cardBaseClasses}
    >
      <div className={`relative overflow-hidden w-full ${project.size === 'tall' ? 'h-64' : 'h-48'}`}>
        <img 
          src={project.image} 
          alt={project.title} 
      ...
      <div className="p-6 flex flex-col flex-1 justify-between bg-surface-variant">
        <div>
          <div className="flex justify-between items-start mb-2">
             ...
             <ArrowUpRight size={20} className="text-on-surface-variant opacity-50 group-hover:opacity-100 group-hover:text-primary group-hover:-translate-y-0.5 transition-all duration-300" />
          </div>
